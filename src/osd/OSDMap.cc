  ...
  map<pg_t, mempool::osdmap::vector<pair<int,int>>> *to_remap) const
{
  ...
  for (auto& pg : to_check) {
    if (!pg_exists(pg)) {
      ...
      up_to_cancel->insert(pg);
      ui_to_cancel->insert(pg);
      continue;
    }
    vector<int> raw, up;
    ...
    if (r < 0) {
      ldout(cct, 0) << __func__ << " verify_up of pg " << pg
                    << " returning " << r
                    << dendl;
      // in general there should be no crush collisions generated by
      // pg_upmaps since they are simply a variant of current up, hence
      // below we blaming pg_upmap_items only
      ui_to_cancel->insert(pg);
    }
    // below we check against crush-topology changing for pg_upmap_items
  ...
  return !up_to_cancel->empty() || !ui_to_cancel->empty() || !to_remap->empty();
}
  ...
  Incremental *pending_inc) const
{
  ...
  clean_pg_upmaps(cct, pending_inc, up_to_cancel, ui_to_cancel, to_remap);
  return any_change;
}
  ...
  OSDMap::Incremental *pending_inc)
{
  ...
  // temporarily drop all primed pg_upmaps to get the real pg distribution
  // this way calc_pg_upmaps could reliably skip any intermediate
  // suboptimal results
  // note that it is okay to keep pg_upmaps for balancing primaires
  // since they have no effect on normal re-balancing procedure
  OSDMap::Incremental temp_inc(epoch + 1);
  temp_inc.fsid = fsid;
  map<pg_t, set<int>> pg_raw_up_sets;
  for (auto& p : pg_upmap) {
    auto& pg = p.first;
    ...
    int primary;
    vector<int> raw_up;
    tmp.pg_to_raw_osds(pg, &raw_up, &primary);
    set<int> raw_up_set(raw_up.begin(), raw_up.end());
    set<int> up_set(p.second.begin(), p.second.end());
    pg_raw_up_sets[pg] = raw_up_set;
    if (up_set == raw_up_set)
      continue;
    temp_inc.old_pg_upmap.insert(pg);
  }
  ...
  map<int, int> change_by_target_osd;
  ...
  test_change:
    ...
    for (auto& i : to_unmap) {
      ...
      auto it = tmp.pg_upmap_items.find(i);
      ceph_assert(it != tmp.pg_upmap_items.end());
      for (auto& p : it->second)
        ++change_by_target_osd[p.first];
      tmp.pg_upmap_items.erase(it);
...
do_upmap:
  ...
  map<int, set<pg_t>> pgs_by_target_osd;
  map<pg_t, set<int>> target_osds_by_pg;
  for (auto& p : pg_upmap) {
    auto& pg = p.first;
    if (!only_pools.empty() && !only_pools.count(pg.pool()))
      continue;
    set<int> up_set(p.second.begin(), p.second.end());
    auto it = pg_raw_up_sets.find(pg);
    if (it == pg_raw_up_sets.end())
      continue;
    set<int> raw_up_set(it->second.begin(), it->second.end());
    if (up_set == raw_up_set) // must be generated by balancer, e.g.,
      continue;               // to balance primary
    set<int> target_osds;
    // raw_up_set - up_set
    std::set_difference(raw_up_set.begin(), raw_up_set.end(),
                        up_set.begin(), up_set.end(),
                        std::inserter(target_osds, target_osds.begin()));
    ldout(cct, 10) << " mon-primed pg " << pg
                   << " target osd(s) " << target_osds
                   << dendl;
    target_osds_by_pg[pg] = target_osds;
    for (auto o : target_osds)
      pgs_by_target_osd[o].insert(pg);
  }
  for (auto p : pgs_by_target_osd) {
    auto osd = p.first;
    ldout(cct, 10) << " drop mon-primed pg_upmap for osd." << osd << dendl;
    for (auto& pg : p.second) {
      if (pending_inc->old_pg_upmap.count(pg))
        continue;
      if (change_by_target_osd[osd] >= target_change)
        break;
      for (auto affected_osd : target_osds_by_pg[pg])
        ++change_by_target_osd[affected_osd];
      pending_inc->old_pg_upmap.insert(pg);
    }
  }
  if (!pgs_by_target_osd.empty()) {
    // return if there are still primed pg_upmaps to process
