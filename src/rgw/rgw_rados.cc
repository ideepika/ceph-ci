					    ...
					    void *_index_op, optional_yield y)
{
  ...
  if (r < 0) {
    ldout(store->ctx(), 10) << __func__
			    << "(): target->get_state() failed with r="
			    << r << dendl;
    return r;
  }
  ...
  auto ref = store->get_obj_head_ref(target->get_bucket_info(), obj, y);
  if (!ref) {
    ldout(store->ctx(), 10) << __func__
			    << "(): store->get_obj_head_ref() got error "
			    << ref.error()<< dendl;
  ...
  if (r < 0) {
    ldout(store->ctx(), 10)
      << __func__ << "(): target->prepare_atomic_modification() failed, r="
      << ref.error()<< dendl;
    return r;
  }
  ...
  if (target->bucket_info.obj_lock_enabled() && target->bucket_info.obj_lock.has_rule() && meta.flags == PUT_OBJ_CREATE) {
    ...
    if (iter == attrs.end()) {
      ...
      op.setxattr(RGW_ATTR_OBJECT_RETENTION, std::move(bl));
    }
  }

  if (state->is_olh) {
    op.setxattr(RGW_ATTR_OLH_ID_TAG, bufferlist(state->olh_tag));
  }

  op.set_mtime(meta.set_mtime);

  if (meta.data) {
    /* if we want to overwrite the data, we also want to overwrite the
       xattrs, so just remove the object */
    op.write_full(bufferlist(*meta.data));
  }
  ...
  if (meta.rmattrs) {
    for (auto iter = meta.rmattrs->begin(); iter != meta.rmattrs->end(); ++iter) {
  ...
  if (meta.manifest) {
    ...
    op.setxattr(RGW_ATTR_MANIFEST, std::move(bl));
  }
  ...
  if (attrs.find(RGW_ATTR_PG_VER) == attrs.end()) {
    R::CLS::rgw::obj_store_pg_ver(op, RGW_ATTR_PG_VER);
  }

  if (attrs.find(RGW_ATTR_SOURCE_ZONE) == attrs.end()) {
    bufferlist bl;
    encode(store->svc.zone->get_zone_short_id(), bl);
    op.setxattr(RGW_ATTR_SOURCE_ZONE, std::move(bl));
  }

  if (!storage_class.empty()) {
    bufferlist bl;
    bl.append(storage_class);
    op.setxattr(RGW_ATTR_STORAGE_CLASS, std::move(bl));
  }
  ...
  if (!index_op->is_prepared()) {
    ...
    if (r < 0) {
      ldout(store->ctx(), 10)
	<< __func__<< "(): index_op->prepare() failed: r=" << r << dendl;
      return r;
    }
  }

  tracepoint(rgw_rados, operate_enter, req_id.c_str());
  auto ec = ref->operate(std::move(op), y, &epoch);

  tracepoint(rgw_rados, operate_exit, req_id.c_str());
  if (ec) { /* we can expect to get -ECANCELED if object was replaced under,
               or -ENOENT if was removed, or -EEXIST if it did not exist
               before and now it does */
    ldout(store->ctx(), 10) << __func__<< "(): ref->operate() failed: ec="
			    << ec << dendl;
    if (ec == bs::errc::file_exists && assume_noent) {
      target->invalidate_state();
      ldout(store->ctx(), 10) << __func__<< "(): invalidating and returning"
			      << ec << dendl;
  ...
  if (r < 0) {
    ldout(store->ctx(), 10) << __func__<< "(): index_op->complete() failed: r="
			    << r << dendl;
    goto done_cancel;
  }
  ...
  if (versioned_op && meta.olh_epoch) {
    ...
    if (r < 0) {
      ldout(store->ctx(), 10) << __func__<< "(): store->set_olh() failed: r="
			      << r << dendl;
      return r;
    }
  }
                           ...
                           RGWRados::AttrsMod attrs_mod)
{
  ...
  case RGWRados::ATTRSMOD_NONE:
    std::copy(src_attrs.begin(), src_attrs.end(),
...
#define MAX_COMPLETE_RETRY 100
  for (i = 0; i < MAX_COMPLETE_RETRY; i++) {
    bool canceled = false;
    // ATTRTODO
    std::map xttr{attrs.begin(), attrs.end()};
               ...
               optional_yield y)
{
  ...
  std::map xttr{attrs.begin(), attrs.end()}; // ATTRTODO
  return processor.complete(accounted_size, etag, mtime, set_mtime, xttr, delete_at,
...
void RGWRados::remove_rgw_head_obj(R::WriteOp& op)
{
  R::CLS::rgw::remove_obj(op, { RGW_ATTR_OLH_PREFIX });
}
...
int RGWRados::Object::Delete::delete_obj(optional_yield y)
{
  ...
  RGWObjState *state;
  int r = target->get_state(&state, false, y);
  if (r < 0)
    return r;
  ...
  if (!real_clock::is_zero(params.unmod_since)) {
    ...
    R::CLS::rgw::obj_check_mtime(op, params.unmod_since, params.high_precision_time,
  ...
  version_t lastv;
  r = ceph::from_error_code(ref->operate(std::move(op), y, &lastv));
  ...
  if (r >= 0) {
    ...
    r = index_op.complete_del(poolid, lastv, state->mtime, params.remove_objs);
...
bs::error_code RGWRados::Object::Stat::stat_async()
{
  ...
  state.completion = ref->aio_operate(std::move(op), nullptr, ba::use_future);
  return {};
}
                                 ...
                                 R::Op& op)
{
  ...
  if (state->obj_tag.length() > 0 && !state->fake_tag) {
    op.cmpxattr(RGW_ATTR_ID_TAG, R::cmpxattr_op::eq, state->obj_tag);
  } else {
...
int RGWRados::Object::prepare_atomic_modification(R::WriteOp& op,
						  bool reset_obj,
						  const string *ptag,
                                                  const char *if_match,
						  const char *if_nomatch,
						  ...
						  optional_yield y)
{
  ...
  if (modify_tail) {
    op.setxattr(RGW_ATTR_TAIL_TAG, std::move(bl));
  }
...
int RGWRados::set_attrs(void *ctx, const RGWBucketInfo& bucket_info,
			rgw_obj& src_obj,
                        bc::flat_map<string, bufferlist>& attrs,
                        ...
                        optional_yield y)
{
  ...
  int r = append_atomic_test(rctx, bucket_info, obj, op, &state, y);
  if (r < 0)
    return r;
  ...
  op.set_mtime(mtime);
  version_t last_v;
  auto ec = ref->operate(std::move(op), y, &last_v);
  if (state) {
    if (!ec) {
      bufferlist acl_bl = attrs[RGW_ATTR_ACL];
      bufferlist etag_bl = attrs[RGW_ATTR_ETAG];
				 ...
				 optional_yield y)
{
  ...
  if (reading_from_head) {
    /* only when reading from the head object do we need to do the
       atomic test */
    r = store->append_atomic_test(&source->get_ctx(), source->get_bucket_info(),
    ...
    if (iter == state.io_ctxs.end()) {
      ...
      if (!ioc) {
        ldout(cct, 20) << "ERROR: failed to open pool context for pool="
		       << read_obj.pool << " r=" << ioc.error() << dendl;
  ...
  auto ec = store->operate(read_obj.oid, *state.cur_ioctx,
			   std::move(op), nullptr, y);
  ldout(cct, 20) << "rados->read r=" << ec << " bl.length=" << bl.length() << dendl;
  if (ec) {
    return ceph::from_error_code(ec);
  }
